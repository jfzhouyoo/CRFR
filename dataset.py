...
class dataset(object):
    def __init__(self,filename,opt):
        ...
        self.subkg=pkl.load(open('data/subkg.pkl','rb'))    
    ...
    def data_process(self,is_finetune=False):
        ...
        for line in self.data:
            ...
            response,r_length,response_concept_mask,_=self.padding_w2v(line['response'],self.max_r_length)
            if False:
                mask_response,mask_r_length,response_concept_mask,_=self.padding_w2v(self.response_delibration(line['response']),self.max_r_length)
            else:
            ...
            data_set.append([np.array(context),c_lengths,np.array(response),r_length,np.array(mask_response),mask_r_length,line['entity'],
                             line['movie'],concept_mask,dbpedia_mask,line['rec'],response_concept_mask])
        return data_set
...
class CRSdataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        '''
        context, c_lengths, response, r_length, mask_response, mask_r_length, entity, movie, concept_mask, dbpedia_mask, rec, response_concept_mask= self.data[index]
        entity_vec = np.zeros(self.entity_num)
        ...
        return context, c_lengths, response, r_length, mask_response, mask_r_length, entity_vec, entity_vector, movie, np.array(concept_mask), np.array(dbpedia_mask), concept_vec, db_vec, rec, np.array(response_concept_mask)
