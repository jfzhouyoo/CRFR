...
class TransformerDecoderLayerKG(nn.Module):
    def __init__(
        ...
        self.norm_user = nn.LayerNorm(embedding_size)
    ...
    def forward(self, x, encoder_output, encoder_mask, kg_encoder_output, kg_encoder_mask, db_encoder_output, db_encoder_mask, user_emb):
        decoder_mask = self._create_selfattn_mask(x)
        user_emb = user_emb.unsqueeze(1).repeat(1,x.shape[1],1)
        # first self attn
        residual = x
        # don't peak into the future!
        x = self.self_attention(query=x, mask=decoder_mask)
        x = self.dropout(x)  # --dropout
        x = x + residual + user_emb 
        x = _normalize(x, self.norm1)
...
class TransformerDecoderKG(nn.Module):
    ...
    def forward(self, input, encoder_state, encoder_kg_state, encoder_db_state, user_emb, incr_state=None):
        encoder_output, encoder_mask = encoder_state
        ...
        for layer in self.layers:
            tensor = layer(tensor, encoder_output, encoder_mask, kg_encoder_output, kg_encoder_mask, db_encoder_output, db_encoder_mask, user_emb)
