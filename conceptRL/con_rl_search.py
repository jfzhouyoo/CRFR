...
def batch_concept_beam_search(concept_env, con_ac_model, user_embed, context_embed, batch_node_set, db_nodes_features, con_nodes_features, device, topk=[5,5,1]):
    def _batch_acts_to_mask(batch_acts):
        batch_masks = []
        for acts in batch_acts:
            num_acts = len(acts)
            act_mask = np.zeros(con_ac_model.act_dim, dtype=np.uint8)
            act_mask[:num_acts] = 1
            batch_masks.append(act_mask)
        return np.vstack(batch_masks)
    
    state_pool,batch_done = concept_env.reset(user_embed, context_embed, batch_node_set, db_nodes_features, con_nodes_features) 
    path_pool = concept_env._batch_path
    ...
    for hop in range(3):
        ...
        if hop==0:
            acts_pool = concept_env._batch_curr_actions
        else:
            acts_pool = concept_env._batch_get_actions(path_pool)
        actmask_pool = _batch_acts_to_mask(acts_pool)
        actmask_tensor = torch.ByteTensor(actmask_pool).to(device)
        
        probs, _ = con_ac_model((state_tensor, actmask_tensor))
        ...
        if hop < 2:
            concept_env._user_embed = 0.5*user_embed[user_id_reference]+0.5*context_embed[user_id_reference]
            
            state_pool = concept_env._batch_get_state(path_pool)
    return path_pool, probs_pool, user_id_reference

def eval_path(user_id_reference,path_pool,probs_pool,topk=10):
    ...
    return original_userid_nodeset,userid_nodeset,sorted_userid_path